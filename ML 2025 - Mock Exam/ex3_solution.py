...
inputs = df["text"].tolist(), padding=True, truncation=True, max_length=128, return_tensors="pt"
...
reg.fit(X, y) #TODO FOR THE STUDENTS 

# Predict
predicted_target = reg.predict(test_embedding.numpy())[0]
print(f"Predicted target for test sentence: {predicted_target:.3f}")
